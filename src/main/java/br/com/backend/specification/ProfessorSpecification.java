...
public class ProfessorSpecification {
    ...
    public static Specification<Professor> withFilters(String name, String email, Boolean active) {
        return (root, query, criteriaBuilder) -> {

            if (Professor.class.equals(query.getResultType())) {
                root.fetch("user", JoinType.LEFT);
            }

            Join<Professor, User> user = root.join("user", JoinType.LEFT);
